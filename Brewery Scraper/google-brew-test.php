...
$check_brewery_exist = "SELECT id FROM `google_places_breweries` WHERE name = '$name'";

$brewery_result = mysqli_query($conn, $check_brewery_exist);
print_r($brewery_result);
if(!$brewery_result || mysqli_num_rows($brewery_result) == 0)
{
    $insert_brewery = "INSERT INTO `google_places_breweries` (`name`, `street number`, `street name`, `city`, `state`, `country`, `zip1`,`latitude`,`longitude`, `phone`, `google id`,`int phone num`,`hours`, `place id`, `price level`, `rating`, `types`, `google_plus`, `website`, `verified`)
